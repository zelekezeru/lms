...
const closeModal = () => {
    showTranscriptsModal.value = false;
};
...
function exportPDF() {
    ...
    doc.setFontSize(16);
    doc.text("Shiloh International Theological Seminary", pageWidth / 2, 20, {
        align: "center",
    });
    doc.setFontSize(14);
    ...
    doc.text(
        `Graduation Date: ${student.graduationDate || "N/A"}`,
        105,
        y + 18
    );
    y += 30;
    ...
    props.semesters.forEach((semester, index) => {
        ...
        const totalCredits = grades.reduce(
            (acc, g) => acc + parseFloat(g.course?.credit_hours || 0),
            0
        );
        ...
        doc.setFontSize(12);
        doc.text(
            `Semester: ${semester.year?.name ?? "Unknown Year"} - ${
        ...
        doc.text(
            `Semester GPA: ${semGPA}`,
            pageWidth - (blockWidth * 2 + marginRight),
            y + 2
        );
        doc.text(
            `Total Credits: ${totalCredits}`,
            pageWidth - (blockWidth + marginRight),
            y + 2
        );
        doc.text(
    ...
    doc.text(
        "Grading Scale: A = 4.0, A- = 3.7, B+ = 3.3, B = 3.0, B- = 2.7, etc.",
        14,
        footerY + 10
    );
    doc.text(
        "This is an official transcript issued by Shiloh International Theological Seminary.",
        14,
        footerY + 16
    );
