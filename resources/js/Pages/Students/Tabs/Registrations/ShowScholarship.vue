...
function requestScholarship() {
    Swal.fire({
        ...
        if (result.isConfirmed && result.value) {
            router.visit(
                route("students.request-scholarship", {
...
function approveScholarship() {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.visit(
                route("students.approve-scholarship", {
                    student: student.value.id,
                }),
                {
                    method: "post",
...
function rejectScholarship() {
    Swal.fire({
        ...
        if (result.isConfirmed && result.value) {
            router.visit(
                route("students.reject-scholarship", {
                    student: student.value.id,
                }),
                {
                    method: "post",
...
<template>
    ...
    >
        ...
        >
            <div
                v-show="
                    userCanAny(['request-scholarship', 'approve-scholarship'])
                "
            ...
            >
                ...
                <template v-if="status.is_scholarship_requested == 1">
                    ...
                    >
                    <div
                        class="w-full mt-2 p-3 bg-yellow-50 dark:bg-yellow-900 border-l-4 border-yellow-400 rounded"
                    >
                        <span
            ...
            <div
                v-show="userCanAny(['view-scholarships'])"
            ...
            >
                <span
                    class="text-sm font-medium text-blue-700 dark:text-blue-300 mb-1 text-center"
                    >Student Scholarship Status</span
                >
                <span
                ...
                >
                    ...
                    <svg
                        v-else
                        class="w-4 h-4 mr-1 text-red-500 dark:text-red-300"
                        fill="none"
                        stroke="currentColor"
                        stroke-width="2"
                        viewBox="0 0 24 24"
                ...
                >
                    <div class="flex flex-col gap-2 my-4 w-full">
                        <span
                            class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-gradient-to-r text-white font-bold shadow-lg border-2 border-green-200 dark:border-green-400"
                        >
                            <AcademicCapIcon class="w-4 h-4" />
                            Requested by:
                            <span class="ml-1">
                                {{
                                    status.scholarship_requested_by_name ||
                                    "N/A"
                                }}
                            </span>
                        </span>
            ...
            <div
                v-show="userCanAny(['approve-scholarship'])"
            ...
            >
                ...
                >
                    <span class="text-sm text-green-500 dark:text-green-400">
                        <CheckIcon class="w-5 h-5 inline" /> Approved by:
                        {{ status.scholarship_approved_by_name }}
                        <br />
                        <ClockIcon class="w-5 h-5 inline" /> Approved on:
                        {{
                            new Date(
                                status.scholarship_approved_at
                            ).toLocaleDateString()
                        }}
                    </span>
                </span>
                ...
                >
                    <span class="text-sm text-green-500 dark:text-green-400">
                        <span class="text-sm text-red-500 dark:text-red-400">
                            <XCircleIcon class="w-5 h-5 inline" /> Rejected by:
                            {{ status.scholarship_rejected_by_name }}
                            <br />
                            <ClockIcon class="w-5 h-5 inline" /> Rejected on:
                            {{
                                new Date(
                                    status.scholarship_rejected_at
                                ).toLocaleDateString()
                            }}
                        </span>
                    </span>
