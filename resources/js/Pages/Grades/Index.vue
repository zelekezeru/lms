...
<template>
    <AppLayout>
                ...
                <tbody>
                    ...
                    >
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <Link :href="route('grades.show', { grade: grade.id })">
                                {{ grade.id }}
                            </Link>
                        </th>
                        <td class="px-6 py-4">{{ grade.name }}</td>
                        <td class="px-6 py-4">{{ grade.value }}</td>
                        <td class="px-6 py-4 flex space-x-6">
                            <Link :href="route('grades.show', { grade: grade.id })" class="text-blue-500 hover:text-blue-700">
                                <EyeIcon class="w-5 h-5"/>
                            </Link>
                            <Link :href="route('grades.edit', { grade: grade.id })" class="text-green-500 hover:text-green-700">
        ...
        <div class="mt-3 flex justify-center space-x-6">
            <Link
                v-for="link in grades.meta.links"
                :key="link.label"
