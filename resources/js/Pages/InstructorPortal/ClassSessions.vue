...
const sessions = computed(() => props.instructor?.classSessions ?? []);
const search = ref("");
const selectedSession = ref(null);

const statusStyle = (status) => {
    const s = status?.toLowerCase();
    if (s === "completed" || s === "done") return "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400";
    if (s === "cancelled" || s === "canceled") return "bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400";
    if (s === "upcoming" || s === "scheduled") return "bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400";
        ...
        <div class="pb-24 md:pb-8 max-w-3xl mx-auto px-4 md:px-6 pt-4 space-y-5">
            ...
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-violet-50 dark:bg-violet-900/20 flex items-center justify-center">
                    <PlayCircleIcon class="w-5 h-5 text-violet-600 dark:text-violet-400" />
                </div>
                <div>
                    <h1 class="text-xl font-bold text-gray-900 dark:text-white">Class Sessions</h1>
                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ sessions.length }} total sessions</p>
                </div>
            </div>
            ...
            <div v-if="filtered.length" class="space-y-3">
                ...
                >
                    ...
                    <div class="p-4 flex items-center gap-4">
                        <div class="w-11 h-11 rounded-xl bg-violet-50 dark:bg-violet-900/20 flex items-center justify-center shrink-0 group-hover:bg-violet-100 dark:group-hover:bg-violet-900/40 transition">
                            <PlayCircleIcon class="w-6 h-6 text-violet-600 dark:text-violet-400" />
                        </div>
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between gap-2">
                                ...
                                <span :class="['text-xs font-semibold rounded-full px-2 py-0.5 shrink-0', statusStyle(session.status)]">
                                    {{ session.status }}
                                </span>
                            </div>
                            <div class="mt-1.5 flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-500 dark:text-gray-400">
                                <span class="flex items-center gap-1"><CalendarIcon class="w-3.5 h-3.5" /> {{ session.date ?? '—' }}</span>
                                <span class="flex items-center gap-1"><ClockIcon class="w-3.5 h-3.5" /> {{ formatTime(session.startTime) }}</span>
                                <span v-if="session.room" class="flex items-center gap-1"><MapPinIcon class="w-3.5 h-3.5" /> {{ session.room.name }}</span>
                            </div>
                        </div>
                        <ChevronRightIcon class="w-4 h-4 text-gray-300 dark:text-gray-600 shrink-0 group-hover:text-violet-500 transition" />
                    </div>
                </button>
            </div>
        ...
        <Teleport to="body">
            ...
            >
                <div v-if="selectedSession" class="fixed inset-0 z-50 flex items-end md:items-center justify-center">
                    <div class="absolute inset-0 bg-black/40 backdrop-blur-sm" @click="selectedSession = null" />
                    <transition
                        enter-active-class="transition duration-300 ease-out"
                        enter-from-class="translate-y-full md:scale-95 opacity-0"
                        ...
                        <div class="relative z-10 w-full md:max-w-lg bg-white dark:bg-gray-900 rounded-t-3xl md:rounded-2xl shadow-2xl overflow-hidden">
                            ...
                            <div class="flex items-center justify-between px-5 pt-5 pb-3 border-b border-gray-100 dark:border-gray-800">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 class="text-sm font-bold text-gray-900 dark:text-white line-clamp-1">{{ selectedSession.course?.name }}</h3>
                                        <span :class="['text-xs font-medium rounded-full px-2 py-0.5', statusStyle(selectedSession.status)]">{{ selectedSession.status }}</span>
                                    </div>
                                </div>
                                <button @click="selectedSession = null" class="p-1.5 rounded-xl text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition">
                            ...
                            <div class="px-5 py-4 grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p class="text-sm font-semibold text-gray-900 dark:text-white flex items-center gap-1.5">
                                        <CalendarIcon class="w-4 h-4 text-gray-400" /> {{ selectedSession.date ?? '—' }}
                                    </p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p class="text-sm font-semibold text-gray-900 dark:text-white flex items-center gap-1.5">
                                        <MapPinIcon class="w-4 h-4 text-gray-400" /> {{ selectedSession.room?.name ?? 'TBD' }}
                                    </p>
                                </div>
                                <div>
                                    <p class="text-xs text-gray-400 uppercase tracking-wider mb-0.5">Instructor</p>
                                    <p class="text-sm font-semibold text-gray-900 dark:text-white flex items-center gap-1.5">
                                        <UserIcon class="w-4 h-4 text-gray-400" /> {{ selectedSession.instructor?.name ?? 'TBA' }}
                                    </p>
                                </div>
                            ...
                            <div class="px-5 pb-6">
                                <button @click="selectedSession = null" class="w-full bg-violet-600 hover:bg-violet-700 text-white rounded-xl py-3 text-sm font-semibold transition">Done</button>
                            </div>
                        </div>
                    </transition>
                </div>
