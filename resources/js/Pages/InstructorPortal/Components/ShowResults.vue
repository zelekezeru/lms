...
const activateWeight = (weightId) => {
  ...
  // Focus first input
  setTimeout(() => {
    const input = document.querySelector(`[data-weight-id="${weightId}"] input`);
    if (input) {
...
const submitWeightResults = () => {
  ...
  if (!weightId) {
    Swal.fire({ icon: "warning", title: "No active weight", text: "Activate a weight column first." });
    return;
  }
  ...
  for (const student of props.studentsList) {
    ...
    if (isNaN(point) || point < 0 || point > weight.point) {
      Swal.fire({ icon: "error", title: "Invalid input", text: `Points for ${student.firstName} must be between 0 and ${weight.point}` });
      return;
    }
    results.push({
  ...
  router.post(route('results.store'), { results }, {
    ...
    onError: () => {
      Swal.fire({ icon: "error", title: "Error", text: "Failed to save results." });
    }
  });
};
</script>
      ...
      <tbody>
        ...
        >
          ...
          >
            ...
            <span v-else>
              <span
                v-if="getStudentResultPoint(student.id, weight.id) !== null"
