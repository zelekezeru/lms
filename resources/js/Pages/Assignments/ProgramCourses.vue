...
const submit = () => {
    ...
    router.post(route("program-courses.attach", props.program.id), form.value);
};
</script>
    ...
    <AppLayout>
        <div>
            <h2 class="text-xl font-semibold text-gray-800 dark:text-gray-200">
                Assign Courses to
                <span class="text-info">{{ program.name }}</span>
            </h2>
        </div>
        ...
        >
            <div class="w-full py-5">
                <label class="block text-gray-100 dark:text-white mb-2"
                    >Select Courses For: <span class="text-info">"{{ program.name }}"</span></label
                >
                <div
